...
def move(dist):
    def inner(old_state, log):
        ...
        cX = old_state.x + dist * math.cos(angle_rads)
        cY = old_state.y + dist * math.sin(angle_rads)
        newX, newY, status = check_position(cX, cY)
        ...
        return new_state, log + [f'POS({int(new_state.x)},{int(new_state.y)}) | FINISHED_STATUS({status})']
...
def set_state(new_mode):
    def inner(old_state, log):
        ...
        return new_state, log + [f'STATE {new_mode} RESOURCES {check_resources(new_mode)}']
